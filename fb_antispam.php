	...
	function comment_post($post_ID) {
		...
		if ((get_option('fbjsas_trackback') == '1') && ($comment_type == 'trackback' || $comment_type == 'pingback' || $comment_type === '')) {
			if( $answer == '' ) {
				...
				wp_die($errors['empty'] .'<br /><br /><strong>Dein Kommentar:</strong><br /><em>'. $comment_content .'</em>');
			} elseif ( $answer != $select_answer ) {
				$this->fbjsas_delete_comment($post_ID);
				wp_die($errors['wrong'] .'<br /><br /><strong>Dein Kommentar:</strong><br /><em>'. $comment_content .'</em>');
			}
		}
		return $post_ID;
	}
...
?>
	...
	<div class="wrap" id="config">		
		...
		<form method="post" id="fbjsas_options" action="">
			<p><strong>Antwort: </strong><small>m&ouml;gliche Antworten,  Trenne mehrere Antworten durch einen Zeilenumbruch. Je komplexer die Antworten, desto schwieriger ist es f&uuml;r den Spambot-Robot, aber auch f&uuml;r den kommentierenden Besucher. Vergebe viele Antworten und es wird per Zufall eines ausgew&auml;hlt. Wenn du keine Antwort vergibst, dann ist die Standardantwort 'Spamschutz'.</small><br /><textarea name="fbjsas_nojsanswer" cols="60" rows="4" id="fbjsas_nojsanswer" style="width: 99%;" ><?php form_option('fbjsas_nojsanswer'); ?></textarea></p>
			<p><strong>Hinweis: </strong><small>Vergebe den Schl&uuml;ssel %word% f&uuml;r das Antwort-Wort und es wird an deiser Stelle deiner Antwort erscheinen.</small><br /><textarea class="code" rows="1" cols="60" name="fbjsas_advice" id="fbjsas_advice" style="width: 99%;" /><?php echo stripslashes(get_option('fbjsas_advice')); ?></textarea></p>
			<p><strong>Leer: </strong><small>Dein Hinweis, wenn das Feld <strong>nicht</strong> ausgef&uuml;llt wurde. xHTML m&ouml;glich</small><br /><textarea class="code" rows="3" cols="60" name="fbjsas_empty" id="fbjsas_empty" style="width: 99%;" /><?php echo stripslashes(get_option('fbjsas_empty')); ?></textarea></p>
			<p><strong>Falsch: </strong><small>Dein Hinweis, wenn das Feld <strong>falsch</strong> ausgef&uuml;llt wurde. xHTML m&ouml;glich</small><br /><textarea class="code" rows="3" cols="60" name="fbjsas_wrong" id="fbjsas_wrong" style="width: 99%;" /><?php echo stripslashes(get_option('fbjsas_wrong')); ?></textarea></p>
			<p><strong>Kommentare sofort L&ouml;schen: </strong><br /><input name="fbjsas_delete" id="fbjsas_delete" value='1' <?php if(get_option('fbjsas_delete') == '1') { echo "checked='checked'";  } ?> type="checkbox" /> <small>Kommentare, die als Spam erkannt werden sofort l&ouml;schen oder als Spam in der Datenbank ablegen? <br /><strong>Achtung (gilt auch f&uuml;r die nachfolgenden Felder Trackback und Pingback)</strong> Aktiviere den Options-Button, wenn du keine Informationen mit Index 'spam' in der Datenbank stehen haben willst. Eintr&auml;ge in der Datenbank, die mit dem Index 'spam' hinterlegt sind, k&ouml;nnen zum Beispiel mit dem Plugin <a href="http://bueltge.de/wp-spamviewer-zum-loeschen-und-retten-von-spam/255" title="zum Plugin">SpamViewer</a> gelistet werden und eventuelle Fehleintr&auml;ge oder Pingbacks k&ouml;nnen gerettet werden. Au&szlig;erdem wird bei nicht aktiviertem Options-Button und falscher Eingabe des Spam-Pr&uuml;ffeldes die wiederholte Eingabe des Kommentares als 'Doppelter Content' angesehen und ist nicht m&ouml;glich, es muss min. ein Zeichen ver&auml;ndert werden.</small></p>
			<p><strong>Trackback sofort L&ouml;schen: </strong><br /><input name="fbjsas_delete_tb" id="fbjsas_delete_tb" value='1' <?php if(get_option('fbjsas_delete_tb') == '1') { echo "checked='checked'";  } ?> type="checkbox" /> <small>Trackbacks, die als Spam erkannt werden sofort l&ouml;schen oder als Spam in der Datenbank ablegen?</small></p>
			<p><strong>Pingback sofort L&ouml;schen: </strong><br /><input name="fbjsas_delete_pb" id="fbjsas_delete_pb" value='1' <?php if(get_option('fbjsas_delete_pb') == '1') { echo "checked='checked'";  } ?> type="checkbox" /> <small>Pingbacks, die als Spam erkannt werden sofort l&ouml;schen oder als Spam in der Datenbank ablegen?</small></p>
			<p><strong>Trackback/Pingback: </strong><br /><input name="fbjsas_trackback" id="fbjsas_trackback" value='1' <?php if(get_option('fbjsas_trackback') == '1') { echo "checked='checked'";  } ?> type="checkbox" /> <small>Trackbacks und Pingbacks nicht filtern und ungepr&uuml;ft passieren lassen.<br />Aktiviere diese Option, wenn du die Option 'Sofort L&ouml;schen' gew&auml;hlt hast, denn sonst werden auch Trackbacks und Pingbacks sofort gel&ouml;scht und nicht in der Datenbank erhalten.</small></p>
			<p class="submit"><input class="submit" type="submit" name="Submit" tabindex="10" value="<?php _e('Update Options'); ?> &raquo;" /></p>
			<input type="hidden" name="page_options" value="'dofollow_timeout'" />
		</form>
		</fieldset>
		<hr />
		<p><small>Weitere Informationen: Besuche die <a href="http://bueltge.de">Plugin-Homepage</a> f&uuml;r zus&auml;tzliche Hinweise und Hintergr&uuml;nde oder hole die letzte Version des Plugins.<br />&copy; Copyright 2007 <a href="http://bueltge.de">Frank B&uuml;ltge</a> | Du willst Danke sagen, dann besuche meine <a href="http://bueltge.de/wunschliste/">Wunschliste</a>.</small></p>
	</div>
...
function fbjsas_update() {
	if (!empty($_POST)) {
		...
		echo '<div class="updated"><p>Die Einstellungen wurden gespeichert.</p></div>';
	}
}
